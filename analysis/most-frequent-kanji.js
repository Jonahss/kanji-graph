...
orderedByFrequency.then(async (results) => {
  ...
  let whereClauseA = topTen.slice(0, topTen.length/2).map((res) => `k.character = '${res['k.character']}'`).join(' OR ')
  let whereClauseB = topTen.slice(topTen.length/2).map((res) => `kk.character = '${res['k.character']}'`).join(' OR ')
  results = await graph.query(`MATCH (kk:kanji)<-[]-(w:word)-[]->(k:kanji) WHERE (${whereClauseA}) AND (${whereClauseB}) RETURN w.kanji, w.meaning`)
